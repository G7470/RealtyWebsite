...
function addReglement( $mode )
{
    ...
    $this->SetFont( "Arial", "B", 10);
    $this->Cell(10,4, "Preferred Method of Payment", 0, 0, "C");
...
{
    $this->SetFont( "Arial", "", 10);
    $length = $this->GetStringWidth( "Remark : " . $remarque );
    $r1  = 10;
    $r2  = $r1 + $length;
    $y1  = $this->h - 45.5;
    $y2  = $y1+5;
    $this->SetXY( $r1 , $y1 );
    $this->Cell($length,4, "Remark : " . $remarque);
}
...
function addCadreEurosFrancs()
{
    ...
    $y1  = $this->h - 45;
    $y2  = $y1+30;
    $this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2-$y1), 5, 'D');
    ...
    /*$this->SetFont( "Arial", "B", 12);
    $this->SetXY( $r1+22, $y1 );
    $this->Cell(15,4, "US Dollars", 0, 0, "C");*/
    /*$this->SetFont( "Arial", "", 12);
    $this->SetXY( $r1+42, $y1 );
    $this->Cell(15,4, "FRANCS", 0, 0, "C");*/
    $this->SetFont( "Arial", "B", 12);
    $this->SetXY( $r1+5, $y1+2 );
    $this->Cell(20,4, "Order Total", 0, 0, "C");
	$this->Line($r1,$y1+6,$r2,$y1+6);
    $this->SetXY( $r1+5, $y1+7 );
    $this->Cell(20,4, "Tax", 0, 0, "C");
	$this->Line($r1,$y1+11,$r2,$y1+11);
    $this->SetXY( $r1+5, $y1+13 );
    $this->Cell(20,4, "Past Paid", 0, 0, "C");
	$this->Line($r1,$y1+17,$r2,$y1+17);
    $this->SetXY( $r1+5, $y1+19 );
    $this->Cell(20,4, "Total Due", 0, 0, "C");
}
...
function displaytotals($ordtotal, $discount)
{
	$total = $ordtotal - $discount;
	$tax = $ordtotal * 0;
	$total = $total + $tax;
	$re  = $this->w - 60;
    $rf  = $this->w - 30;
	...
	//$fmt->formatCurrency(100, 'US');
    $this->Cell( 17,4, $fmt->formatCurrency($ordtotal, 'USD'), '', '', 'R');
    $this->SetXY( $re, $y1+7 );
    $this->Cell( 17,4, "+ " . $fmt->formatCurrency($tax, 'USD'), '', '', 'R');
    $this->SetXY( $re, $y1+13 );
    $this->Cell( 17,4, "- " . $fmt->formatCurrency($discount, 'USD'), '', '', 'R');
	$this->SetFont( "Arial", "", 14);
    $this->SetXY( $re, $y1+19 );
    $this->Cell( 17,4, $fmt->formatCurrency($total, 'USD'), '', '', 'B');
}
